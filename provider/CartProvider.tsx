...
const CartProvider: React.FC<{children: React.ReactNode}> = ({children}) => {
    ...
    useEffect(()=>{
        ...
        const newTotal = cartItems.reduce((total,items)=> total + items.price * items.quantity ,0 );
    ...
    function removeItemFromCart(item: CartItem, quantity: number = 1) {
        ...
        if (quantity === -1) quantity = updatedCartItems[existingItemIndex].quantity;
        if (existingItemIndex !== -1) {
    ...
    return (
        <CartContext.Provider value={{cartItems, setCartItems, addProductToCart, removeItemFromCart, cartBadge, previousCartBadge,cartTotal, cartTotalAbrivated}}>
            {children}
